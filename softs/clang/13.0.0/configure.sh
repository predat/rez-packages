...
echo -e "[CONFIGURE][ARGS] LLVM VERSION: ${LLVM_VERSION}"

# We check if the arguments variables we need are correctly set.
# If not, we abort the process.
if [[ -z ${EXTRACT_PATH} || -z ${BUILD_PATH} || -z ${INSTALL_PATH} || -z ${LLVM_VERSION} ]]; then
    echo -e "\n"
...
# We run the configuration script of LLVM.
echo -e "\n"
echo -e "[CONFIGURE] Running the configuration script from LLVM-${LLVM_VERSION}..."
echo -e "\n"
