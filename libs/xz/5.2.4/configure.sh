...
XZ_VERSION=${REZ_BUILD_PROJECT_VERSION}
...
echo -e "[CONFIGURE][ARGS] XZ VERSION: ${XZ_VERSION}"

# We check if the arguments variables we need are correctly set.
# If not, we abort the process.
if [[ -z ${EXTRACT_PATH} || -z ${BUILD_PATH} || -z ${INSTALL_PATH} || -z ${XZ_VERSION} ]]; then
    echo -e "\n"
...
# We run the configuration script of xz.
echo -e "\n"
echo -e "[CONFIGURE] Running the configuration script from xz-${XZ_VERSION}..."
echo -e "\n"
