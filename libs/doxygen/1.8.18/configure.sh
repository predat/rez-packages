...
echo -e "[CONFIGURE][ARGS] DOXYGEN VERSION: ${DOXYGEN_VERSION}"

# We check if the arguments variables we need are correctly set.
# If not, we abort the process.
if [[ -z ${EXTRACT_PATH} || -z ${BUILD_PATH} || -z ${INSTALL_PATH} || -z ${DOXYGEN_VERSION} ]]; then
    echo -e "\n"
...
# We run the configuration script of Doxygen.
echo -e "\n"
echo -e "[CONFIGURE] Running the configuration script from Doxygen-${DOXYGEN_VERSION}..."
echo -e "\n"
