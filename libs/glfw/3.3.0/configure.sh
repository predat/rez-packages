...
GLFW_VERSION=${REZ_BUILD_PROJECT_VERSION}
...
echo -e "[CONFIGURE][ARGS] GLFW VERSION: ${GLFW_VERSION}"

# We check if the arguments variables we need are correctly set.
# If not, we abort the process.
if [[ -z ${EXTRACT_PATH} || -z ${BUILD_PATH} || -z ${INSTALL_PATH} || -z ${GLFW_VERSION} ]]; then
    echo -e "\n"
...
# We run the configuration script of GLFW.
echo -e "\n"
echo -e "[CONFIGURE] Running the configuration script from GLFW-${GLFW_VERSION}..."
echo -e "\n"
